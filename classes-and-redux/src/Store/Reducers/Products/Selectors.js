const sorterFactory = (sortOptions, productsMap) => {
    ...
    const compareStringFields = (a, b) => {
        const aObj = productsMap.get(a);
        const bObj = productsMap.get(b);
...
export const filterProductIds = state => {
    ...
    const filteredIds = filter ? state.products.ids.filter(x => productsMap.get(x).name.toLowerCase().includes(filter.toLowerCase())) : state.products.ids;
